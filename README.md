...
### مرحله 4️⃣: راه‌اندازی کامل دیتابیس (یک دستور!)

اکنون با **یک دستور** تمام کارها را انجام دهید:

```bash
npm run pb:setup-all
```

این دستور به صورت خودکار موارد زیر را انجام می‌دهد:
- ✅ ایجاد تمام collections (categories, products, cart_items, orders)
- ✅ تنظیم فیلدهای اضافی برای کاربران (users)
- ✅ تنظیم قوانین دسترسی برای تمام collections
- ✅ وارد کردن 5 دسته‌بندی و محصولات نمونه

**یا** اگر می‌خواهید مرحله به مرحله پیش بروید:
...
### مرحله 5️⃣: ساخت اکانت کاربری و ادمین کردن آن
...
# دستورات PocketBase
npm run pb:setup-all         # راه‌اندازی کامل (توصیه می‌شود!)
npm run pb:setup             # فقط ایجاد collections
npm run pb:seed              # فقط وارد کردن داده‌های نمونه
npm run pb:fix-rules         # فقط تنظیم قوانین دسترسی
...
1. دستور `npm run pb:setup-all` را مجدداً اجرا کنید (یا `npm run pb:seed` + `npm run pb:fix-rules`)
2. صفحه مرورگر را رفرش کنید (F5)
